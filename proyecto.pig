...
-- Get average time for each category for each person
...
-- Get average time for each category for each country
...
-- Store
STORE order_by_ext INTO '/uhadoop2020/pbelmonte/results/time/order_by_ext/';
STORE order_by_est INTO '/uhadoop2020/pbelmonte/results/time/order_by_est/';
STORE order_by_agr INTO '/uhadoop2020/pbelmonte/results/time/order_by_agr/';
STORE order_by_csn INTO '/uhadoop2020/pbelmonte/results/time/order_by_csn/';
STORE order_by_opn INTO '/uhadoop2020/pbelmonte/results/time/order_by_opn/';

---------------------------------------------------------------------------------------------
-- SCORE

-- Get average score for each category for each person
prom_person = FOREACH filtered GENERATE country, (EXT1 + EXT2 + EXT3 + EXT4 + EXT5 + EXT6 + EXT7 + EXT8 + EXT9 + EXT10) / 10.0 AS EXT_PROM, (EST1 + EST2 + EST3 + EST4 + EST5 + EST6 + EST7 + EST8 + EST9 + EST10) / 10.0 AS EST_PROM, (AGR1 + AGR2 + AGR3 + AGR4 + AGR5 + AGR6 + AGR7 + AGR8 + AGR9 + AGR10) / 10.0 AS AGR_PROM, (CSN1 + CSN2 + CSN3 + CSN4 + CSN5 + CSN6 + CSN7 + CSN8 + CSN9 + CSN10) / 10.0 AS CSN_PROM, (OPN1 + OPN2 + OPN3 + OPN4 + OPN5 + OPN6 + OPN7 + OPN8 + OPN9 + OPN10) / 10.0 AS OPN_PROM;
